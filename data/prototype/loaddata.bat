...
for /R %~dp0 %%i IN (*.shp) do (
  ...
  -sql "INSERT INTO data.elements_in (elt_id, source_id, source_elt_id, geom) SELECT elt_id::varchar, source_id, source_elt_id::varchar, ST_SetSrid(wkb_geometry, 0) FROM data.temp_elements"
...
%OGRBIN%\ogrinfo -ro PG:%PGCONN% -sql "SELECT m1.gen_topo_update()"
