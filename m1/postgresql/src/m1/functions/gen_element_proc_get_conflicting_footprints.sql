CREATE OR REPLACE FUNCTION m1.gen_element_proc_get_conflicting_footprints(aelm_proc_id integer, max_clearance double precision DEFAULT 0)
  ...
  RETURN QUERY
    ...
    , elements AS (
      ...
      WHERE ep.elm_proc_id <> f.elm_proc_id
        ...
        -- distance by clearance
        AND ST_DWithin(epf.geom, f.geom, COALESCE(r.clearance, 0)) 
    )
