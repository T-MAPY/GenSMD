...
BEGIN
  ...
  RETURN QUERY (
    ...
    COALESCE(ST_Equals(
      m1.gen_create_footprint(
        ...
        '{"buffer": {"radius": 2, "endcap": "round"}}'::jsonb
      ),
      ST_Buffer('LINESTRING(0 0, 10 0)'::geometry, 2)
    ), false) AS result
  );
  ...
  RETURN QUERY (
    ...
    COALESCE(ST_Equals(
      m1.gen_create_footprint(
        'LINESTRING(0 0, 10 0)'::geometry, 
        '{"buffer": {"radius": 2, "endcap": "flat"}}'::jsonb
      ),
  ...
  RETURN QUERY (
    ...
    COALESCE(ST_Equals(
      ...
      ST_Buffer('POLYGON((0 1,1 1,1 2,0 2,0 1))'::geometry, 1)
    ), false) AS result
  );
