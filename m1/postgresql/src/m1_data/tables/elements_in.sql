...
CREATE TABLE elements_in (
    ...
    CONSTRAINT check_topogeom_topo_ln CHECK ((((topo_ln).topology_id = 7) AND ((topo_ln).layer_id = 2) AND ((topo_ln).type = 2))),
    CONSTRAINT check_topogeom_topo_pl CHECK ((((topo_pl).topology_id = 7) AND ((topo_pl).layer_id = 3) AND ((topo_pl).type = 3))),
    CONSTRAINT check_topogeom_topo_pt CHECK ((((topo_pt).topology_id = 7) AND ((topo_pt).layer_id = 1) AND ((topo_pt).type = 1)))
);
