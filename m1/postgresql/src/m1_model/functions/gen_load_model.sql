CREATE OR REPLACE FUNCTION m1_model.gen_load_model(xmldoc text)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
  ...
  sqlinsert text;
BEGIN
  m := m1_utils.xmldoctojsondoc(xmldoc)::jsonb;
  -- remove root
  m := jsonb_extract_path(m, (SELECT jsonb_object_keys(m) LIMIT 1));
  --RAISE NOTICE '%', m;
  
  -- truncate old data
  TRUNCATE m1_model.element_types_load;
  TRUNCATE m1_model.element_types_relations_load;
  
  -- load element types
  INSERT INTO m1_model.element_types_load (id, source, merged)
  ...
  LOOP
    ...
    LOOP
      BEGIN
        sqlinsert := 'INSERT INTO m1_model.element_types_relations_load (id_from, id_to, merged)
                      SELECT %fields%, $1 
                      FROM 
                        (SELECT * FROM m1_model.vw_element_types_load WHERE ' || rsel.cond || ') selector
                        INNER JOIN 
                        (SELECT * FROM m1_model.vw_element_types_load WHERE ' || rrel.cond || ') relation
                        ON ' || rrel.condjoin || '
    ...
    IF (rsel.elementtype IS NOT NULL) THEN
      BEGIN
        EXECUTE 'UPDATE m1_model.element_types_load SET merged = $1
                 WHERE id IN (SELECT id FROM m1_model.vw_element_types_load WHERE ' || rsel.cond || ')'
